...
> Status: **brewing** — not planned for implementation. This document exists to capture
> direction and open questions so the idea ripens over time. Update it as thinking evolves.
> Promote it to "ready for implementation" only after the cloud-sync API layer is live.
